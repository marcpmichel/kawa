...
class SExpressionVisitor : ASTVisitor {
    Stack!string stack;
    ...
    override void visit(BinaryOpNode node) {
        ...
        stack.push(format("(%s %s %s)", node.operator, leftResult, rightResult));
    }
    ...
    override void visit(VariableDeclarationNode node) {
        if(node.value !is null) {
            ...
            stack.push(format("(declare %s:%s %s)", node.name, node.type, valueResult));
        }
        else {
            stack.push(format("(declare %s)", node.name));
        }
    }
    ...
    override void visit(IfNode node) {
        ...
        if(node.elseBranch is null) {
            ...
            stack.push(format("(if %s %s %s)", conditionResult, thenResult, elseResult));
        }
    }
    ...
    override void visit(FunctionDeclarationNode node) {
        ...
        node.body.accept(this);
        string bodyResult = stack.pop();
        stack.push(format("(defun %s (%s) %s)", node.name, paramsResult, bodyResult));
    }
    ...
    override void visit(PodDeclaration node)  {
        ...
        import std.array: join;
        string paramsResult = params.join(" "); 
        stack.push(format("pod %s ( %s )", node.name, paramsResult)); 
    }

    override void visit(Parameter node) {
        /* stack.push("[function parameter]"); */
        stack.push(format("%s:%s", node.name, node.type));
    }
}
